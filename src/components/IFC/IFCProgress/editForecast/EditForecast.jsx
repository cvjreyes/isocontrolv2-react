...
import EditForecastHead from "../../../FEED/feedProgress/editForecast/EditForecastHead";
import EditForecastRow from "../../../FEED/feedProgress/editForecast/EditForecastRow";
...
function EditForecastComp({ setMessage, setModalContent }) {
  ...
  const deleteEntry = async (week) => {
    const { ok } = await api("delete", `/ifd/delete_forecast/${week}`);
    if (ok) {
      // slice
      const tempData = [...data];
